...
const SERVER_URL =
  process.env.NODE_ENV === 'production' ? REMOTE_SERVER : LOCAL_SERVER;
...
class HttpService {
  ...
  static async createBoard(
    ...
    try {
      ...
      const createdBoard: Omit<BoardType, 'columns'> =
      ...
      const columns = await Promise.all(
        createdColumns.map(
          async (c): Promise<Omit<ColumnType, 'tasks'>> => c.json(),
        ),
      );
  ...
  static async getColumn(
    ...
    try {
      const response = await fetch(
        `${SERVER_URL}boards/${boardId}/columns/${columnId}`,
        {
          method: 'GET',
          headers: { Authorization: `Bearer ${getTokenFromCookie()}` },
        },
  ...
  static async updateColumn(
    ...
    try {
      ...
      const response = await fetch(
        `${SERVER_URL}boards/${boardId}/columns/${id}`,
        {
          method: 'PUT',
          headers: {
            Accept: 'application/json',
  ...
  static async createTask(
    task: Omit<TaskType, 'id,files'>,
  ): Promise<Omit<TaskType, 'files'>> {
    try {
      const { title, done, order, description, userId, boardId, columnId } =
        task;
      const response = await fetch(
        `${SERVER_URL}boards/${boardId}/columns/${columnId}/tasks`,
        {
  ...
  static async updateTask(
    task: Omit<TaskType, 'files'>,
  ): Promise<Omit<TaskType, 'files'>> {
    try {
      const { id, title, done, order, description, userId, boardId, columnId } =
        task;
      const response = await fetch(
